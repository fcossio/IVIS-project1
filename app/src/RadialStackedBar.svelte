<!-- obtained from https://svend3r.dev/charts/radialStacked -->
...
<script>
    ...
    const width = 1279; // width of inner radius inverted, in pixels
    const innerRadius = 180; // radius of inner circle, in pixels
    const colorRange = ['#98abc5','#7b6888','#a05d56','#d0743c','#ff8c00']; // fill colors for each bar stack - MUST match number of datasets
    ...
    let groupId;
    $: groupId = keys[0];
    $: reactiveData = data.sort((a, b) => b.total - a.total)
